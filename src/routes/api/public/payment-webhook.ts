  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // UTMify is NOT notified from here on purpose. The external checkout
        // platform already reports every order to UTMify itself (with the
        // real product name and the ad-attribution codes). Sending it from
        // this webhook too created a duplicate "Venda pendente" for each
        // order, without attribution, doubling revenue in UTMify.
